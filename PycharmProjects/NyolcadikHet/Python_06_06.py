...
class LinkedList:           # LáncoltLista osztály
    ...
    def insertAtPosition(self,data,n): # HelyreBeszúr művelet: az n. pozícióra szúrja be az elemet
        ...
        if self.head==None or n<=1:    # ha a lista üres vagy a pozíció legfeljebb 1, akkor az
             ...
             if self.tail==None:       # ha eddig üres volt a lista, akkor
                 self.tail=self.head   # a tail-t az új, egyetlen elemre kell állítani"""
        if n <= 1:
            self.insertAtBeginning(data)  # Használjuk az insertAtBeginning függvényt
    ...
    def insertAtIndex(self, data, index):
        if index < 0 or index > self.counter:
            print(f"Hibás index: {index}. Az indexnek 0 és {self.counter} között kell lennie.")
            self.error = True
            return
        if index == 0:
    ...
    def removeFirstNode(self):
        if self.head==None:         # üres listából nem lehet elemet törölni,
            self.error=True         # ezért a hibát igazra állítjuk
        else:                           # ekkor a lista nem üres:
            self.head=self.head.next    # a következő csomópont lesz az új fej (vagy None)
            self.counter-=1             # elemszám csökkentése
            if self.counter==0:         # ha üres lett a lista, akkor
                self.tail=self.head     # a tail is None
            self.error=False            # nem volt hiba
    ...
    def Counting(self, data):  # Új Counting függvény
        count = 0
        tmp = self.head
        while tmp:
            if tmp.data == data:
...
lista.removeFirstNode()# első csomópont törlése - ha lehetne
# ha a lista üres
if lista.error:
    print("A lista üres, így nem törölhető belőle elem!")
print()
lista.insertAtBeginning("répa")# elejére a "répa" elem
print("Egy elem van:")
lista.printList()
lista.removeLastNode()# utolsó csomópont törlése
lista.printList()
print("A fejelem törlése után üres lett a lista.")
print()
print("4 elem felvétele (mindig a lista végére beszúrva):")
lista.insertAtEnd("sárgarépa") # "sárgarépa"
lista.insertAtEnd("sárgarépa")
lista.insertAtEnd("karalábé") # "karalábé"
lista.insertAtEnd("petrezselyem") # "petrezselyem"
lista.insertAtEnd("paprika") # "paprika"
lista.printList()
print()
lista.insertAtPosition("paradicsom", 20) # "paradicsom" beszúrása a 20. pozícióra
print("A paradicsom beszúrása után:")
lista.printList()
print()
# 1. Feladat tesztelése: Counting
print(f"A 'sárgarépa' elemek száma a listában: {lista.Counting('sárgarépa')}")

# 2. Feladat tesztelése: insertAtPosition
print("\n'A paprika' beszúrása az első pozícióra:")
lista.insertAtPosition("paprika", 1)
lista.printList()

# 3. Feladat tesztelése: insertAtIndex
print("\n'A cékla' beszúrása a 3. indexre:")
lista.insertAtIndex("cékla", 3)
lista.printList()

print("\nHibás index megadása (pl. -1):")
lista.insertAtIndex("uborka", -1)