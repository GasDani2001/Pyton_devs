...
class BinaryTree:                 # BinFa, azaz bináris fa
    ...
    def partOfBinTree(self,bintree,direction):
        if bintree.root == None and self.act != None: # ha bintree üres és az act létező csomópont:
                if direction == "balra": # ha az irány "balra":
                    bintree.root= self.act.left # bintree gyökere legyen az act balgyereke
                if direction == "jobbra": # ha az irány "jobbra":
                    bintree.root = self.act.right # bintree gyökere legyen az act jobbgyereke
