...
class LinkedList:           # LáncoltLista osztály
    ...
    def reverseOrder(self): # feltételeznünk kell, hogy véget ér, azaz a lista nem tartalmaz hurkot
        if self.head==None:# leállási feltétel: már nincs több Csomópont, az alapeset az üres lista
            return         
        tmp_head=self.head # leválasztjuk a fejet
        tmp_remainder=LinkedList()        # létrehozunk egy segédlistát, amely kezdetben üres
        tmp_remainder.head= self.head.next# a segédlista fejét a maradék listára állítjuk
        tmp_remainder.reverseOrder()# kiíratjuk a maradék listát visszafelé
