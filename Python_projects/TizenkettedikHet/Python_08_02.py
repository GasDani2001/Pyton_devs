...
class BinarySearchTree:         # BinKerFa, azaz bináris keresőfa
    ...
    def ReverseOrder(self,node):
        result=[]
        if node:
            result=self.ReverseOrder(node.right)
            result.append(node.data)
            result+=self.ReverseOrder(node.left)
        return result
...
kerfa.printBinTree()
print()
print("Az elemek fordított, azaz csökkenő sorrendben:")
for item in kerfa.traversalReverseOrder():
    print(item,end=' ')
